...
local function switchItem(tool, delayTime)
	...
	if LongjumpProjectileInUse then
		pcall(function() tool = getItem("fireball").tool end)
	end
...
local LongjumpProjectileInUse = false
...
run(function()
	...
	local FlightInterface = Instance.new("ScreenGui", gethui())
	local lastFlightPercentRemoved = tick()
	...
	Fly = vape.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			...
			if callback then
				...
				lastFlightPercentRemoved = tick()

				if FlightInterface then
					FlightInterface:Destroy()
				end

				FlightInterface = Instance.new("ScreenGui", gethui())

				local FlightBar = Instance.new("Frame", FlightInterface)
				FlightBar.AnchorPoint = Vector2.new(0.5, 0.5)
				FlightBar.Size = UDim2.fromScale(0.13,0.015)
				FlightBar.Position = UDim2.fromScale(0.5,0.7)
				FlightBar.BorderSizePixel = 0
				FlightBar.BackgroundColor3 = Color3.fromRGB(40,40,40)

				local FlightTime = Instance.new("TextLabel", FlightInterface)
				FlightTime.AnchorPoint = Vector2.new(0.5, 0.5)
				FlightTime.Size = UDim2.fromScale(0.13,0.015)
				FlightTime.Position = UDim2.fromScale(0.5,0.65)
				FlightTime.BackgroundTransparency = 1
				FlightTime.Text = "2.5"
				FlightTime.TextSize = 14
				FlightTime.TextColor3 = Color3.fromRGB(255,255,255)
				--FlightTime.TextStrokeSize = 1
				FlightTime.TextStrokeTransparency = 0

				local FlightBarPercent = Instance.new("Frame", FlightBar)
				FlightBarPercent.Size = UDim2.fromScale(1,1)
				FlightBarPercent.BorderSizePixel = 0
				FlightBarPercent.BackgroundColor3 = Color3.fromRGB(255,255,255)

				local FlightTimeLeft = 2.5
				...
				Fly:Clean(runService.PreSimulation:Connect(function(dt)
					if entitylib.isAlive and not InfiniteFly.Enabled and isnetworkowner(entitylib.character.RootPart) then
						...
						if FlightTimeLeft > 2.5 or workspace.Gravity < 196 then
							FlightTimeLeft = 2.5
						end

						if FlightTimeLeft < 0 then
							FlightTimeLeft = 0
						end
						
						local scaledValue = FlightTimeLeft / 2.5
						local clampedValue = math.floor(scaledValue * 100) / 100
						
						tweenService:Create(FlightBarPercent, TweenInfo.new(0.05), {
							Size = UDim2.fromScale(scaledValue, 1)
						}):Play()
						
						FlightTime.Text = string.format("%.2f", FlightTimeLeft)
						
						FlightTimeLeft -= tick() - lastFlightPercentRemoved
						lastFlightPercentRemoved = tick()
...
run(function()
	...
	function spoofupwards(duration)

		if vape.Modules.Invisible and vape.Modules.Invisible.Enabled then
			vape.Modules.Invisible:Toggle()
			notif('AnticheatBypass', 'Invisible cannot be used with AnticheatBypass', 3, 'warning')
		end
		...
		AnticheatBypass:Clean(runService.PreSimulation:Connect(function(dt)
			if entitylib.isAlive then
				local dist = (lplr.Character.PrimaryPart.Position - oldroot.Position).Magnitude

				if (dist > 6 or tick() - lastUpdate > 0.15) then
					if (isnetworkowner(oldroot)) then
						oldroot.Transparency = 0
						oldroot.Anchored = false
						oldroot.CFrame = lplr.Character.PrimaryPart.CFrame
					else
						lplr.Character.PrimaryPart.CFrame = oldroot.CFrame
					end
					lastUpdate = tick()
				else
		...
		if success and clone and oldroot and proper then
			...
			notif('AnticheatBypass', 'Waiting 1.1s to disable', 1.1)
			task.delay(1.1, function()
				notif('AnticheatBypass', 'Disabled!', 1)
				connection:Disconnect()
				proper = true
...
run(function()
	...
	writefile("b.lua", tostring(vape.Libraries.auraanims))
		...
		Function = function(callback)
			if callback then
				...
				repeat
						...
						if #plrs > 0 then
							...
							for _, v in plrs do
								...
								if actualRoot then
									...
									pcall(function()
										if pauraenabled and #getProjectiles(true) > 0 then
											task.spawn(function()
												task.wait(MultiDelay.Value / 2000)
												projaurashoot(true, MultiKnockback.Enabled)
												task.wait(MultiDelay.Value / 2000)
											end)
										end
									end)
					...
					if Face.Enabled and attacked[1] and tick() - spawnedAt > 3 then
	...
	MultiDelay = Killaura:CreateSlider({
		Name = 'Multi Delay (MS)',
		Min = 1,
		Max = 1000,
		Default = 250,
	...
	MultiKnockback = Killaura:CreateToggle({Name = 'Multi Knockback'})
...
run(function()
	...
	local LongJumpMethods = {
		...
		fireball = function(item, pos)
			LongjumpProjectileInUse = true
			launchProjectile(item, pos, 'fireball', 60)
			task.delay(0.5, function()
				LongjumpProjectileInUse = false
			end)
		end,
	...
	LongJump = vape.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			...
			if callback then
				...
				local shouldFall = (HeightValue.Value / 150) / 2
				local lastDirection = lplr.Character.Humanoid.MoveDirection
				...
				LJConnection = runService.PreSimulation:Connect(function(dt)
					local root = entitylib.isAlive and entitylib.character.RootPart or nil
					if root and isnetworkowner(root) then
						if JumpTick > tick() then
							local spd = getSpeed()

							local velo = Direction * (spd + ((JumpTick - tick()) > 1.1 and JumpSpeed or 0))
							if entitylib.character.Humanoid.FloorMaterial == Enum.Material.Air and not start then
								root.AssemblyLinearVelocity = velo + Vector3.new(0, HeightValue.Value - runningticks, 0)
							else
								root.AssemblyLinearVelocity = Vector3.new(root.AssemblyLinearVelocity.X, HeightValue.Value , root.AssemblyLinearVelocity.Z)
							end
							start = nil
							runningticks += shouldFall
							lastDirection = lplr.Character.Humanoid.MoveDirection
						else
				...
				if not othermethod then
					local startTick = tick()
					pcall(function()
						LJConnection:Disconnect()
						--lplr.Character.PrimaryPart.AssemblyLinearVelocity:Destroy()
					end)
					task.spawn(function()
						repeat
							local dir = lplr.Character.Humanoid.MoveDirection
							local speed = getSpeed()
							lplr.Character.PrimaryPart.AssemblyLinearVelocity = Vector3.new(dir.X * speed,HeightValue.Value - runningticks,dir.Z * speed)
							runningticks += shouldFall
							task.wait()

							if tick() - startTick > 2.2 then
								LongJump:Toggle()
							end
							
						until tick() - startTick > 2.4 or not LongJump.Enabled
					end)
				end
			else
				JumpTick = tick()
	...
	HeightValue = LongJump:CreateSlider({
		Name = 'Height',
		Min = 0,
		Max = 50,
		Default = 25,
...
run(function()
	...
	getProjectiles = function(exclude)
		local items = {}
		for _, item in store.inventory.inventory.items do
			...
			if ammo and table.find(List.ListEnabled, ammo) then
				...
				if exclude and ammo:lower():find("snow") then
					continue
				end
	...
	projaurashoot = function(fromaura, snowballs)

		if fromaura == nil then
			fromaura = false
		end

		if snowballs == nil then
			snowballs = true
		end

		local ent = entitylib.EntityPosition({
			Part = 'RootPart',
			Range = fromaura and 9e9 or Range.Value,
			Players = Targets.Players.Enabled,
			NPCs = Targets.NPCs.Enabled,
			Wallcheck = Targets.Walls.Enabled
		})

		local ent2 = entitylib.EntityPosition({
			Part = 'RootPart',
			Range = 16,
			Players = Targets.Players.Enabled,
			NPCs = Targets.NPCs.Enabled,
		...
		if not fromaura and ent2 then
			return
		end

		if ent then
			local pos = entitylib.character.RootPart.Position
			for _, data in getProjectiles(snowballs) do
	...
	ProjectileAura = vape.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			if callback then
				pauraenabled = callback
				repeat
					if (workspace:GetServerTimeNow() - bedwars.SwordController.lastAttack) > 0.5 and not Attacking then
						projaurashoot()
					end
					task.wait(0.1)
					...
					if entitylib.isAlive then
						...
						if pearl and root.Velocity.Y < -100 then
							if not check then
								...
								rayCheck.FilterDescendantsInstances = {lplr.Character, AntiFallPart or Instance.new("Part")}

								local ground = getNearGround(20)
								local ray = workspace:Raycast(root.Position, Vector3.new(0, -1000, 0), rayCheck)
	
								if ground and (ray == nil) then
...
run(function()
	...
	local function attemptBreak(tab, localPosition, burst)
		...
		for _, v in tab do
			if (v.Position - localPosition).Magnitude < Range.Value and bedwars.BlockController:isBlockBreakable({blockPosition = v.Position / 3}, lplr) then
				...
				if burst and tick() - lastBurst > 1 then
					for i = 1,10 do
						bedwars.breakBlock(v, Effect.Enabled, Animation.Enabled, CustomHealth.Enabled and customHealthbar or nil, true)
					end
				end
			...
			if callback then
				...
				repeat
					...
					if entitylib.isAlive then
						...
						if attemptBreak(Bed.Enabled and beds, localPosition, Burst.Enabled) then continue end
						if attemptBreak(customlist, localPosition, Burst.Enabled) then continue end
						if attemptBreak(LuckyBlock.Enabled and luckyblock, localPosition, Burst.Enabled) then continue end
						if attemptBreak(IronOre.Enabled and ironores, localPosition, Burst.Enabled) then continue end
	...
	Burst = Breaker:CreateToggle({Name = 'Burst'})
...
run(function() -- Damc im pretty sure you cant disable it once enabled per server, maybe its just me
	local Connection
    InfJump = vape.Categories.Custom:CreateModule({
        Name = "Infinite Jump",
            ...
            if callback then
                ...
                Connection = game:GetService("UserInputService").JumpRequest:Connect(function()
                    if InfiniteJumpEnabled then
                        ...
                        if humanoid then
                            lplr.Character.PrimaryPart.Velocity = Vector3.new(lplr.Character.PrimaryPart.Velocity.X,InfJumpVelocity.Value,lplr.Character.PrimaryPart.Velocity.Z)
                        end
                    end
                end)
			else
	...
	InfJumpVelocity = InfJump:CreateSlider({
		Name = 'Velocity',
		Min = 1,
		Max = 100,
		Default = 50
...
run(function()
    ...
    ExpDetect = vape.Categories.Custom:CreateModule({
        Name = "Exploit Detector",
        Tooltip = "Attempts to detect other exploiters in your server",
        Function = function(callback)
            if callback then
                local ExploitDetectorData = {}

                task.spawn(function()
                    repeat
                        for _, v in pairs(playersService:GetPlayers()) do
                            pcall(function()
                                if v == lplr or not v.Character or not v.Character:FindFirstChild("HumanoidRootPart") or not v.Character:FindFirstChild("Humanoid") or table.find(ExpDetectWhitelist.ListEnabled, v.Name) or table.find(ExpDetectWhitelist.ListEnabled, v.DisplayName) then
                                    return
                                end
                                ...
                                if not ExploitDetectorData[v.UserId] then
                                    ExploitDetectorData[v.UserId] = {
                                ...
                                local data = ExploitDetectorData[v.UserId]
								local damage = data.LastHealth - humanoid.Health
                                local distance = (Vector3.new(data.LastPosition.X, hrp.Position.Y, data.LastPosition.Z) - hrp.Position).Magnitude

                                if distance > humanoid.WalkSpeed * 0.6 and damage <= 0 and ExpDetectSpeed.Enabled then
                                    data.SpeedViolations += 1
                                else
                                    data.SpeedViolations = 0
                                end

                                if data.SpeedViolations > 20 then

									if data.Flags.Speed == nil then
										data.Flags.Speed = 0
									end

									if tick() - data.Flags.Speed > ExpDetectTimer.Value then
										notif('Exploit Detector', v.DisplayName .. " has been detected for Speed!", ExpDetectTimer.Value, 'warning')
										data.Flags.Speed = tick()
									end
                                end

								if hrp.Position.Y > 10000 and ExpDetectinfFlight.Enabled then
									if data.Flags.InfiniteFly == nil then
										data.Flags.InfiniteFly = 0
									end

									if tick() - data.Flags.InfiniteFly > ExpDetectTimer.Value then
										notif('Exploit Detector', v.DisplayName .. " has been detected for InfiniteFly!", ExpDetectTimer.Value, 'warning')
										data.Flags.InfiniteFly = tick()
									end
								end

								if v.AccountAge < 3 and ExpDetectACCAGE.Enabled then
									if data.Flags.AccountAge == nil then
										data.Flags.AccountAge = 0
									end

									if tick() - data.Flags.AccountAge > ExpDetectTimer.Value then
										notif('Exploit Detector', v.DisplayName .. " has been detected for AccountAge (age: "..v.AccountAge..")!", ExpDetectTimer.Value, 'warning')
										data.Flags.AccountAge = tick()
									end
								end

								local onground = humanoid.FloorMaterial ~= Enum.Material.Air

								local airTime = tick() - data.LastGround
								local distFallen = data.LastPosition.Y - hrp.Position.Y

								if airTime > 1.5 and distFallen < 0 and damage <= 0 and ExpDetectSpeed.Enabled and workspace.Gravity > 196 then
									if data.Flags.Flight == nil then
										data.Flags.Flight = 0
									end

									if tick() - data.Flags.Flight > ExpDetectTimer.Value then
										data.Flags.Flight += 1
										if data.Flags.Flight > 3 then
											notif('Exploit Detector', v.DisplayName .. " has been detected for Flight!", ExpDetectTimer.Value, 'warning')
											data.Flags.Flight = tick()
										end
									end
								else 
									data.Flags.Flight = 0
                        ...
                        task.wait(0.5)
                    until not ExpDetect.Enabled
                end)
            end
        end
    })

	ExpDetectTimer = ExpDetect:CreateSlider({
	...
	ExpDetectWhitelist = ExpDetect:CreateTextList({
		Name = 'Whitelist',
		Function = function()
	...
	ExpDetectSpeed = ExpDetect:CreateToggle({
		Name = 'Speed Check',
		Default = true
	})

	ExpDetectflight = ExpDetect:CreateToggle({
		Name = 'Flight Check',
		Default = true
	})

	ExpDetectinfFlight = ExpDetect:CreateToggle({
		Name = 'Infinite Flight Check',
		Default = true
	})

	ExpDetectACCAGE = ExpDetect:CreateToggle({
